...
M3-CSR 12月2 模型更新
    对于 VideoEncoder:
        - 引入 ResNet18 预训练权重并支持冻结骨干; 输入按数据集通道均值/方差归一化
        - 归一化参数使用 MicroLens-50k 统计值, 计算脚本见 tools/image_channel_stats.py
        - 输出帧级特征后做时间维平均池化得到视频向量

    对于 AudioEncoder:
        - 使用了 https://github.com/harritaylor/torchvggish 链接中的方法, 应用本地加载的预训练VGGish模型
        - 
