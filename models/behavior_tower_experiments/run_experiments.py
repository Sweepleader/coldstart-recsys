...
def evaluate_tower(tower, histories, test_df, num_items, device='cpu', k=10, num_neg=100, max_len=50, pool='last', decay=None):
    recalls = []
    ndcgs = []
...
def parse_args():
    ...
    p.add_argument('--tower', type=str, default='baseline', choices=['baseline','improved'])
...
def main():
    ...
    num_users = num_users
    histories = build_histories(args.train_path, num_users)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    if args.tower == 'baseline':
        from baseline_tower import BehaviorTowerBaseline
        tower = BehaviorTowerBaseline(num_items, emb_dim=args.emb_dim).to(device)
    else:
        from improved_tower import BehaviorTowerImproved
        tower = BehaviorTowerImproved(num_items, emb_dim=args.emb_dim, dropout=0.1).to(device)
    ...
    def bpr_step(batch):
        ...
        for st in seq_tensors:
            if args.pool == 'mean' and args.tower == 'improved':
                lengths = torch.tensor([st.size(1)], dtype=torch.long, device=device)
                uv = tower(st, lengths=lengths, pool='mean', decay=args.decay)
            else:
                uv = tower(st)
            uvecs.append(uv)
    ...
    recall, ndcg = evaluate_tower(tower, histories, test_df, num_items, device=device, k=args.k, num_neg=args.num_neg, max_len=args.max_len, pool=args.pool, decay=args.decay)
    ...
    if args.save_csv:
        row = [{
            ...
            'tower': args.tower,
            'pool': args.pool,
            'decay': args.decay if args.decay is not None else '',
