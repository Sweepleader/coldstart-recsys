...
def main():
    ...
    frames_dir = _repo_root() / 'test_file' / 'picture'
    try:
        ...
        for p in sorted(frames_dir.glob('*.jpg')):
            ...
            imgs.append(x)
        if len(imgs) >= 5:
            x = torch.stack(imgs[:5], dim=0).unsqueeze(0)
            frames = x
        else:
            frames = _make_video_input(batch=1, frames=5, h=224, w=224)
    except Exception:
        frames = _make_video_input(batch=1, frames=5, h=224, w=224)
    ...
    try:
        ...
        texts = lines[:1] if lines else _make_texts(batch=1)
    except Exception:
        texts = _make_texts(batch=1)
